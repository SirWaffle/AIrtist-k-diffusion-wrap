    ...
    def __call__(
        ...
        **kwargs,
        ...
        if "execution_provider" in kwargs:
            ...
            unet_sess = ort.InferenceSession("E:/MLModels/stableDiffusion/sd-v1-4-onnx/unet.onnx", so, providers=[ep])
            post_quant_conv_sess = ort.InferenceSession("E:/MLModels/stableDiffusion/sd-v1-4-onnx/post_quant_conv.onnx", so, providers=[ep])
            decoder_sess = ort.InferenceSession("E:/MLModels/stableDiffusion/sd-v1-4-onnx/decoder.onnx", so, providers=[ep])
            encoder_sess = ort.InferenceSession("E:/MLModels/stableDiffusion/sd-v1-4-onnx/encoder.onnx", so, providers=[ep])
...
image = pipe(prompt, num_inference_steps=12, execution_provider="CUDAExecutionProvider")["sample"][0]
image.save("Dml_1.png")

prompt = "dragons breathing fire, 4k, 8k"
image = pipe(prompt, guidance_scale=4.0, execution_provider="CUDAExecutionProvider")["sample"][0]
image.save("Dml_2.png")

prompt = "beautiful ice dragon sitting on an iceberg, large scaley blue dragon with wings, resting on an iceberg, digital art by peter mohrbacher, peter elson, jesper ejsing"
image = pipe(prompt, height=512, width=512, num_inference_steps=12, execution_provider="CUDAExecutionProvider")["sample"][0]
image.save("Dml_3.png")

prompt = "3D avatar of mark zuckerberg, 4k , 8k"
image = pipe(prompt,  height=512, width=512, guidance_scale=4.0, num_inference_steps=10, execution_provider="CUDAExecutionProvider")["sample"][0]
