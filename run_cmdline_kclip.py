...
def DoGenerate(kdiffReq): 
    ...
    genParams.CLIPprompts = ['A mysterious orb by Ernst Fuchs'] #NONE
    ...
    #DPM_2 / DPM_2_A .. works for RDM
    #LMS seems to work for clip guidance
    ...
    genParams.range_scale = 100           # 50 - Controls how far out of range RGB values are allowed to be.
    genParams.cutn = 128                  # 16 - The number of random crops per step.
                                # Good values are 16 for 256x256 and 64-128 for 512x512.
    genParams.cut_pow = 0.5               # 0.5 - 
    genParams.seed = 1000 #None
    ...
    genParams.overall_clip_scale = 0.8

    genParams.conditioning_scale = 8.0 #??? 
    genParams.sampleMethod = "DPM_2_A"#"HEUN" #"LMS" #LMS or HEUN

    genParams.noiseSchedule = "MODEL"#"KARRAS" #"MODEL"
    genParams.sigma_min = -1.0 #1.4
    genParams.sigma_max = 20 #-1.0 #20.0
    ...
    #modelNum = 7 #7
    clipModelNum = 1#3 #3 #3 for RDM  #0 for kdiff LMS

    kdiffer = kdiffWrap.KDiffWrap()

    clipguided = genParams.clip_guidance_scale != 0

    clipwrap, modelwrap = kdiffer.CreateModels(clipModelNum, 1) #0=RDM, 1=UNCOND
